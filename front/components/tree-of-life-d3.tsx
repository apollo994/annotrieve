...
import { getFlattenedTree } from "@/lib/api/taxons"
  ...
  useEffect(() => {
    async function fetchTreeData() {
      try {
        ...
        const data = await getFlattenedTree()
        
        // Convert to flat array format for d3.stratify()
        const flatNodes: FlatTreeNode[] = data.rows.map((row: any[]) => ({
          id: row[0], // taxid
          parentId: row[1] || null, // parent_taxid
          scientific_name: row[2] || `Taxon ${row[0]}`,
          annotations_count: row[3] || 0,
          assemblies_count: row[4] || 0,
          organisms_count: row[5] || 0,
        }))
